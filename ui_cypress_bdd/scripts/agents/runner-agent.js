'use strict';
// ============================================================
// Runner Agent — Exécution des tests Cypress/Cucumber
// ============================================================
...
// ── Exécution Cypress ───────────────────────────────────────────────────────────
function runCypress(tags = '') {
  const args = ['cypress', 'run', '--spec', 'cypress/e2e/features/**/*.feature'];
  if (tags) args.push('--env', `tags=${tags}`);
...
async function llmSummary(stats, failures) {
  ...
  try {
    const prompt = `Tu es un expert QA. Génère un résumé court (2-3 phrases) du run de tests Cypress.
...
async function cmdCritical() {
  console.log(`\n${B}=== RUNNER — CRITICAL ===${E}`);
  const code = runCypress('@critical');
  const results = loadResults();
...
async function main() {
  ...
  switch (cmd) {
    ...
    default:
      console.log(`
${B}Runner Agent${E} — Exécution des tests Cypress/Cucumber
