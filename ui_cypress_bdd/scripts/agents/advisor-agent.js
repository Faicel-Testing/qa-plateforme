...
async function cmdAdvise(nVotes = N_VOTES) {
  ...
  const _tpl = promptStore.get('release_vote') ||
    'Tu es un Release Manager expert. Décide si ce build Cypress est prêt pour la production.\n\n' +
    'Métriques de test:\n' +
  ...
  for (const [tc, stats] of Object.entries(recurring).slice(0, 10)) {
    ...
    try {
      const _tpl = promptStore.get('predict_gate') ||
        'Prédit le risque d\'échec futur pour ce test Cypress.\n\n' +
        'Test : {test_name}\nHistorique :\n{context}\n' +
...
async function cmdGate() {
  ...
  try {
    const prompt = `Évalue la qualité de ce build Cypress et donne un score 0-100.
...
function cmdMemory(action) {
  if (action === 'recurring') {
    ...
    const demos = [
      ...
      { tc: 'Todo - ajout timeout',           category: 'flaky',    confidence: 0.78 },
      { tc: 'Signup - chargement lent',       category: 'env_issue',confidence: 0.65 },
    ];
