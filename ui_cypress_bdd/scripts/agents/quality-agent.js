...
async function cmdTriage() {
  ...
  for (const f of failures) {
    ...
    const _tpl = promptStore.get('triage_classify') ||
      'Tu es un expert QA. Classifie cet échec de test Cypress.\n\n' +
    ...
    const prompt = fmt(_tpl, {
      ...
      screenshot_path: 'N/A', browser: 'chrome', current_url: 'N/A',
    });
...
async function cmdRca() {
  ...
  for (const [key, group] of Object.entries(groups)) {
    ...
    const _rcaTpl = promptStore.get('rca_analyze') ||
      'Tu es un expert QA Cypress. Analyse la cause racine de ces échecs.\n\n' +
    ...
    const prompt = fmt(_rcaTpl, {
      ...
      error_message: key, stack_trace: failList, suite: 'cypress',
...
async function cmdVerify() {
  ...
  const checks = [
    { name: 'Features Gherkin', path: FEATURES_DIR, ext: '.feature' },
    { name: 'Step Definitions', path: STEPS_DIR,    ext: '.js' },
