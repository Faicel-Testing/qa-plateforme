...
const REPORT_DIR   = path.join(FRAMEWORK, 'allure-report');
const DOCS_DIR     = path.join(FRAMEWORK, 'docs');
const HISTORY_DIR  = path.join(FRAMEWORK, 'allure-history');
...
<html lang="fr">
<head><meta charset="UTF-8"><title>KPI Dashboard — Cypress BDD</title>
...
<body>
<h1>📊 KPI Dashboard — Cypress BDD</h1>
...
async function cmdNotify(target = 'slack') {
  ...
  let summary;
  try {
    const prompt = `Tu es un expert QA. Génère un résumé Slack concis (2-3 phrases) du run Cypress.
...
Ton: direct. Si PASSED → rassurant. Si FAILED → factuel, action concrète.`;
    ...
    summary = `Run Cypress : ${kpi.pass_rate}% de réussite (${kpi.passed}/${kpi.total}).`;
  }
  ...
  const payload = target === 'teams'
    ? { '@type':'MessageCard','@context':'http://schema.org/extensions', themeColor: kpi.gate_ok?'00b894':'d63031',
        summary: `${emoji} Cypress — ${kpi.pass_rate}%`,
        sections:[{ activityTitle:`${emoji} Cypress BDD — ${kpi.pass_rate}% passé`, activitySubtitle: summary,
    ...
    : { attachments:[{ color: kpi.gate_ok?'#22c55e':'#ef4444', blocks:[
        { type:'header', text:{type:'plain_text', text:`${emoji} Cypress BDD — ${kpi.pass_rate}% passed`}},
...
function persistHistory() {
  ...
  for (const name of fs.readdirSync(src)) {
    const s = path.join(src, name), d = path.join(HISTORY_DIR, name);
    if (fs.statSync(s).isFile()) fs.copyFileSync(s, d);
  }
  console.log(`  ${G}Historique Trend sauvegardé : allure-history/${E}`);
}

function cmdHistorySave() {
  persistHistory();
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'history':   cmdHistorySave();        break;
    default:
      console.log(`
${B}Reporting Agent${E} — KPI, Dashboard, Notifications, Sync
  ...
  history          Sauvegarde allure-report/history/ -> allure-history/ (persistant)
