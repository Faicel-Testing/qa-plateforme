# ============================================================
# Runner Agent — Exécution des tests Selenium BDD (Maven)
# ============================================================
# Commandes :
...
#   python agents/runner-agent.py chrome [env]      → ChromeRunnerTest
#   python agents/runner-agent.py firefox [env]     → FirefoxRunnerTest
#   python agents/runner-agent.py parallel [env]    → ParallelRunnerTest
...
import sys, os, json, glob, subprocess, time, xml.etree.ElementTree as ET, argparse
...
def mvn_runner(runner_class: str, env_name: str = "local") -> int:
    return mvn([f'-Dtest={runner_class}'], env_name)
...
def cmd_regression(env: str = "local"):
    print(f"\n{W}RUNNER — Régression @regression (env={env}){E}")
    rc = mvn_tag("@regression", env)
    results = load_results()
    if results:
        print_stats(parse_stats(results))
    return rc
...
def cmd_chrome(env: str = "local"):
    print(f"\n{W}RUNNER — Chrome (env={env}){E}")
    return mvn_runner("ChromeRunnerTest", env)


def cmd_firefox(env: str = "local"):
    print(f"\n{W}RUNNER — Firefox (env={env}){E}")
    return mvn_runner("FirefoxRunnerTest", env)


def cmd_parallel(env: str = "local"):
    print(f"\n{W}RUNNER — Parallèle Chrome + Firefox (env={env}){E}")
    return mvn_runner("ParallelRunnerTest", env)


def cmd_flaky(runs: int = 3, env: str = "local"):
    print(f"\n{W}RUNNER — Détection flaky ({runs} runs, env={env}){E}")
    history = {}  # {test_name: [statuses]}
    for i in range(runs):
        print(f"\n  {C}Run {i+1}/{runs}{E}")
        mvn([], env)
    ...
    flaky = {n: statuses for n, statuses in history.items()
    ...
    if flaky:
        ...
        messages = [{"role": "user", "content": (
            f"Ces tests Selenium BDD sont flaky ({runs} runs) :\n{flaky_text}\n\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Runner Agent — Selenium BDD Maven")
    ...
    elif cmd == "chrome":     cmd_chrome(env)
    elif cmd == "firefox":    cmd_firefox(env)
    elif cmd == "parallel":   cmd_parallel(env)
