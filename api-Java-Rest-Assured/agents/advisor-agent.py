...
def cmd_release(n_votes: int = 3):
    ...
    _tpl = _ps.get("release_vote") or (
        "Tu es QA Lead senior. Évalue si ce build API RestAssured peut être déployé en production.\n\n"
        "{context_str}\n\n"
        "Critères de blocage :\n"
        "- Pass rate < 90%\n"
        "- Taux d'échec > 5%\n"
...
def cmd_predict():
    ...
    _tpl = _ps.get("predict_gate") or (
        "Analyse les tendances de cette suite de tests API RestAssured et prédis :\n\n"
        "{context_str}\n\n"
...
def cmd_recommend():
    ...
    messages = [{"role": "user", "content": (
        f"Analyse cette suite de tests API RestAssured :\n\n{ctx_str}\n\n"
        f"Application testée : restful-booker.herokuapp.com (API réservation hôtelière, 51 scénarios)\n"
        f"Stack : Java 17, RestAssured, Cucumber 7, TestNG\n\n"
...
<title>Advisor Report — API RestAssured</title>
...
<h1>📊 Advisor Report — api-Java-Rest-Assured</h1>
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Advisor Agent — API RestAssured")
