# ============================================================
# Pipeline Agent — Orchestrateur maître api-Java-Rest-Assured
# ============================================================
# Commandes :
#   python agents/pipeline-agent.py full        → planning→run→quality→bug→report→advisor→ci
...
def cmd_full():
    _separator("PIPELINE FULL — api-Java-Rest-Assured")
    start = time.time()
    steps = [
        ("1/7  Planning    ", "planning-agent.py", ["coverage"]),
        ("2/7  Run tests   ", "runner-agent.py",   ["run"]),
        ("3/7  Quality     ", "quality-agent.py",  ["analyze"]),
        ("4/7  Bug triage  ", "bug-agent.py",      ["triage"]),
        ("5/7  Reporting   ", "reporting-agent.py",["publish"]),
        ("6/7  Advisor     ", "advisor-agent.py",  ["release"]),
        ("7/7  CI commit   ", "ci-agent.py",       ["commit"]),
    ]
    results = {}
...
def cmd_status():
    ...
    print(f"""
  {W}api-Java-Rest-Assured — restful-booker.herokuapp.com{E}
  ────────────────────────────────────────
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Pipeline Agent — Orchestrateur api-Java-Rest-Assured")
