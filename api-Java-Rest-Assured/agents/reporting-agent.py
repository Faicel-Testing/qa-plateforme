...
HISTORY_DIR = os.path.join(FRAMEWORK, "allure-history")  # persistant, hors target/ (survit a "mvn clean")
...
def persist_history():
    """Sauvegarde target/allure-report/history/ -> allure-history/ (persistant, hors target/).
    AllureSuiteListener la recopie dans target/allure-results/history/ au run suivant,
    ce qui permet au Trend Allure de survivre a "mvn clean"."""
    src = os.path.join(REPORT_DIR, "history")
    ...
    for name in os.listdir(src):
        ...
        if os.path.isfile(s):
            shutil.copy2(s, d)
    print(f"  {G}Historique Trend sauvegardé : allure-history/{E}")
...
<title>Reporting Dashboard — api-Java-Rest-Assured</title>
<style>
...
<h1>📊 Reporting Dashboard — API RestAssured</h1>
<p style="color:#8b949e">restful-booker.herokuapp.com · {time.strftime('%Y-%m-%d %H:%M')}</p>
...
def cmd_notify(channel: str = "slack"):
    ...
    _tpl = _ps.get("qa_notify") or (
        "Rédige une notification QA courte pour ce run API RestAssured :\n\n"
    ...
    try:
        ...
        narrative = f"Run API RestAssured : {stats['pass_rate']}% pass ({stats['passed']}/{stats['total']})"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Reporting Agent — API RestAssured")
