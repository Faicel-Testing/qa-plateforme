...
#   python agents/quality-agent.py verify         → cohérence features/steps/client
...
def cmd_analyze():
    print(f"\n{W}QUALITY AGENT — Analyse qualité API RestAssured{E}\n")
    ...
    if failures:
        ...
        messages = [{"role": "user", "content": (
            f"Suite de tests API RestAssured — résumé qualité :\n\n"
...
def cmd_kpi():
    ...
    clients  = glob.glob(os.path.join(CLIENT_DIR, "*.java"))

    gate_color = "#2ecc71" if gate_ok else "#e74c3c"
    html = f"""<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8">
<title>KPI Dashboard — api-Java-Rest-Assured</title>
<style>
...
<h1>🧪 KPI Dashboard — API RestAssured (restful-booker.herokuapp.com)</h1>
...
<div class="grid">
  ...
  <div class="card"><div class="metric" style="color:#79c0ff">{len(clients)}</div><div class="label">API Clients</div></div>
</div>
</body></html>"""
...
def cmd_flaky(runs: int = 3):
    ...
    if flaky:
        ...
        _tpl = _ps.get("flaky_analyze") or (
            "Ces tests API RestAssured sont flaky :\n{flaky_list}\n\n"
            "En 3 points, explique les causes (API publique partagée, timing, données concurrentes) "
...
def cmd_verify():
    print(f"\n{W}QUALITY AGENT — Vérification cohérence features / steps / client{E}\n")
    features = glob.glob(os.path.join(FEATURES_DIR, "*.feature"))
    ...
    if scenarios and step_patterns:
        context = f"Features : {len(features)} | Scénarios : {len(scenarios)} | Steps : {len(step_patterns)} | Clients : {len(clients)}"
        messages = [{"role": "user", "content": (
            f"Vérifie la cohérence de ce framework API RestAssured/Java :\n\n{context}\n\n"
            f"Premiers scénarios : {', '.join(scenarios[:5])}\n"
            f"Premiers patterns : {', '.join(step_patterns[:5])}\n\n"
            f"En 3 points, identifie les risques de cohérence (steps non implémentés, endpoints manquants, etc.)."
        )}]
        try:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Quality Agent — API RestAssured")
