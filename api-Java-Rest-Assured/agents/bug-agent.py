# ============================================================
# Bug Agent — Triage · RCA · Réparation API RestAssured
# ============================================================
# Commandes :
...
TRIAGE_SCHEMA = {
    "type": "object",
    "properties": {
        "category":           {"type": "string", "enum": ["real_bug", "flaky", "env_issue", "false_positive", "assertion_issue", "timing_issue"]},
...
RCA_SCHEMA = {
    ...
    "properties": {
        ...
        "cause_category": {"type": "string", "enum": ["assertion", "contract", "timing", "data", "network", "config", "java_exception", "unknown"]},
        "chain":          {"type": "array", "items": {"type": "string"}},
        "affected_layer": {"type": "string", "enum": ["feature", "step_definition", "client", "payload", "api", "infrastructure"]},
...
def load_source_index() -> dict:
    index = {}
    for directory in [CLIENT_DIR, STEPS_DIR]:
...
def classify_failure(failure: dict) -> dict:
    _tpl = _ps.get("triage_classify") or (
        "Analyse cet échec de test API RestAssured et classe-le dans UNE seule catégorie :\n\n"
        "Test    : {test_name}\n"
        "Feature : {feature}\n"
        "Statut  : {status}\n"
        ...
        "Trace   : {stack_trace}\n\n"
        "Catégories (API/Java) :\n"
        "  real_bug         → Bug dans l'API testée\n"
        "  assertion_issue  → Assertion trop stricte ou contrat API différent de l'attendu\n"
        "  timing_issue     → Problème réseau/latence\n"
        "  flaky            → Test instable (API publique partagée, données concurrentes)\n"
    ...
    category = "unknown"
    for cat in ("assertion_issue", "timing_issue", "real_bug", "flaky", "env_issue", "false_positive"):
...
def cmd_triage(name_filter: str = None):
    print(f"\n{W}BUG AGENT — Triage API (Confidence Scoring){E}")
...
def run_rca(failure: dict, all_tests: list) -> dict:
    ...
    _rca_tpl = _ps.get("rca_analyze") or (
        "Effectue une RCA de cet échec de test API RestAssured (Java) :\n\n"
        "Test    : {test_name}\n"
        "Feature : {feature}\n"
        "Statut  : {status}\n"
        ...
        "Identifie la couche affectée : feature | step_definition | client | payload | api"
    )
    ...
    struct_messages = [{"role": "user", "content": (
        f"Sur la base de cette RCA API :\n\n{cot_reasoning}\n\n"
...
def cmd_rca(name_filter: str = None):
    print(f"\n{W}BUG AGENT — RCA API (Chain of Thought){E}\n")
    failures = load_failures(name_filter)
    if not failures:
...
def generate_patch(failure: dict, source_index: dict) -> dict:
    ...
    _patch_tpl = _ps.get("repair_patch") or (
        "Analyse cet échec de test API Java (RestAssured) et propose un correctif :\n\n"
        "Test    : {test_name}\n"
        "Feature : {feature}\n"
        "Message : {error_message}\n"
        "Trace   :\n{stack_trace}\n\n"
        ...
        "le code à remplacer et le nouveau code. Privilégie l'ajustement de l'assertion "
        "au comportement réel de l'API plutôt qu'un contournement fragile."
    )
...
def cmd_loop(name_filter: str = None, max_iter: int = DEFAULT_MAX_ITER):
    ...
    while failures and iter_count < min(max_iter, HARD_CAP):
        ...
        real_bugs = [t for t in triaged if t["category"] in ("real_bug", "assertion_issue", "timing_issue")]
        if not real_bugs:
...
def cmd_report():
    ...
    rows = ""
    for r in triaged:
        cat_color = {"real_bug": "#e74c3c", "assertion_issue": "#e67e22", "timing_issue": "#f39c12",
    ...
    <title>Bug Report — API RestAssured</title>
    ...
    <body><h1>🐛 Bug Report — api-Java-Rest-Assured</h1>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bug Agent — API RestAssured")
