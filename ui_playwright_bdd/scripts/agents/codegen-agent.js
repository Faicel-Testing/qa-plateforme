...
// ============================================================
// Absorbe : generate-agent, spec-agent, tc-generator
//
// Usage:
//   node scripts/agents/codegen-agent.js spec           Génère .feature depuis US Jira
//   node scripts/agents/codegen-agent.js steps          Génère step definitions TypeScript
//   node scripts/agents/codegen-agent.js pages          Génère page objects TypeScript
...
const STEPS_DIR    = path.join(FRAMEWORK, 'src', 'steps');
const PAGES_DIR    = path.join(FRAMEWORK, 'src', 'pages');
...
// ── steps : génère step definitions TypeScript ────────────────────────────────
async function cmdSteps() {
  ...
  if (!features.length) { console.log(`  ${Y}⚠  Dossier src/features/ vide${E}`); return; }
  ...
  for (const featureFile of items) {
    ...
    try {
      const prompt = `Tu es un expert Playwright + Cucumber.js TypeScript. Génère les step definitions pour ce .feature.

${content}

Contraintes strictes:
- import { Given, When, Then, Before, After } from '@cucumber/cucumber'
- import { ICustomWorld } from '../../core/world'
- Toutes les fonctions sont async
- Utilise this.page (Playwright Page) depuis World
- Utilise expect de @playwright/test pour les assertions
- Chaque step sur 1 ligne de code max si possible

Réponds UNIQUEMENT avec le TypeScript complet (pas de markdown).`;

      const resp = await llm.chat([{ role: 'user', content: prompt }]);
      let code   = (resp.message.content || '').replace(/```typescript\n?|```ts\n?|```\n?/g, '').trim();
      span.response = code; span.end(true);

      const filepath = path.join(STEPS_DIR, `${base}.steps.ts`);
      if (DRY_RUN) {
        console.log(`  ${Y}[DRY-RUN]${E} ${path.basename(filepath)}\n`);
      } else {
...
// ── pages : génère page object models ─────────────────────────────────────────
async function cmdPages() {
  ...
  for (const featureFile of items) {
    ...
    console.log(`  ${B}[${featureFile}]${E} → ${className}.ts`);

    const span = new tracer.Span('chatPages', content, llm.MODEL).begin();
    try {
      const prompt = `Tu es un expert Playwright TypeScript POM. Génère un Page Object Model pour ce .feature.
...
- Locators en tant que propriétés readonly (page.locator, getByRole, getByText…)
- Méthodes async pour chaque action (navigate, fill, click, assert)
- import { Page, expect } from '@playwright/test'
- Aucun test() ni test.step()

Réponds UNIQUEMENT avec le TypeScript complet.`;
...
async function main() {
  ...
  switch (cmd) {
    ...
    default:
      console.log(`
${B}Codegen Agent${E} — Génération de code BDD Playwright

  spec           Génère .feature depuis les User Stories Jira
  steps          Génère step definitions TypeScript depuis .feature
  pages          Génère page objects TypeScript depuis .feature
